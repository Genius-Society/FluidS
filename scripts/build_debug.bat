...
set QT_DIR=D:\ProgramData\Qt5.7.1\5.7\msvc2015
...
set SRC_DIR=%cd%
set BUILD_DIR=%cd%\SmokeSimulation
...
%QT_DIR%\bin\qmake.exe %SRC_DIR%\fluids.pro -spec win32-msvc2015 "CONFIG+=debug" "CONFIG+=console"
if exist %BUILD_DIR%\Debug\fluids.exe del %BUILD_DIR%\Debug\fluids.exe
nmake Debug
if not exist %BUILD_DIR%\Debug\Qt5Cored.dll (
  %QT_DIR%\bin\windeployqt.exe %BUILD_DIR%\Debug\fluids.exe
)
